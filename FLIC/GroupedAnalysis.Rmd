...
Other aspects of the data can also be saved to a *.csv* file in the same manner through their corresponding *.Monitors()* functions, such as the durations or intervals between events:

```{r}
OutputDurationData.Monitors(monitors.single,p.single)
OutputDurationData.Monitors(monitors.choice,p.choice.list)
```
...
The same plots are available for Events and for mean Event Durations, rather than Licks, and they are accompanied by a summary of one-way ANOVA results as well. As of 4/20/2020 ANOVA results will fail for analysis of mean Duration if intervals have zero events.
...
To visualize binned data, use *Feeding.BinnedLicksPlot.Trt()*, *Feeding.BinnedEventsPlot.Trt()*, and *Feeding.BinnedDurationsPlot.Trt()* with the following arguments.
...
The same plots can be produced for events and durations, rather than licks.
...
```{r, fig.height=5.5}
Feeding.BinnedDurationsPlot.Trt(monitors.single,p.single,binsize.min=30,expDesign)
```
...
```{r, fig.height=5.5, fig.width=8}
Feeding.BinnedDurationsPlot.Trt(monitors.choice,p.choice.list,binsize.min=30,expDesign)
```


The mean/average duration of each feeding interaction event over the entire experiment can be displayed for each treatment. 
```{r}
...
PlotTotalLicks.Monitors(monitors, p, range=c(0,0),TransformLicks=TRUE,filename="LicksPlots")

OutputBaselinedData.Monitors(monitors,parameters,range=c(0,0),filename="Baselined")

OutputIntervalData.Monitors(monitors,parameters,expDesign=NA,range=c(0,0),filename="IntervalData")

OutputDurationData.Monitors(monitors,parameters,expDesign=NA,range=c(0,0),filename="DurationData")

OutputTotalFeeding.Monitors(monitors,parameters,expDesign=NA,range=c(0,0),filename="TotalFeedingTime")
...
Feeding.BinnedDurationsPlot.Trt(monitors,parameters,binsize.min=20,expDesign,range=c(0,0),SaveToFile=FALSE)
```