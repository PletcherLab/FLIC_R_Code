...
p.choice<-SetParameter(p.choice,Tasting.Threshold.Interval=c(10,20))
```
...
```{r}
summary.onewell<-Feeding.Summary.DFM(dfm.single,range=c(0,120),TransformLicks=FALSE) # Summarize the first two hours, and don't transform.
...
For choice experiments, the results are as follows:
...
The *Feeding.Summary.DFM* function summarizes a range of metrics over the entire range specified.  If you would like to parse the range into non-overlapping bins and to evaluate the feeding interactions in each bin, there is *BinnedFeeding.Summary.DFM(dfm,binsize.min=30,range=c(0,0),TransformLicks=TRUE)*, which takes one additional parameter that specifies the size of each bin, in minutes.

```{r}
binsummary.onewell<-BinnedFeeding.Summary.DFM(dfm.single,binsize.min=10,range=c(0,120),TransformLicks=FALSE) # Summarize the first two hours, and don't transform.
